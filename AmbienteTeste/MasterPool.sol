...
contract MasterLending is Ownable {
    ...
    constructor(address _USDCAddress, address _poolBorrower, uint256 _interestRate, uint256 _paymentFrequency, uint256 _lateFee, uint256 _poolLimit, address _portfolioMulti, address _scalablePool)  {
    ...
    function repayLoan(uint256 _id, uint256 USDCAmount) public onlyContractEnabled() {
        ...
        if((Loans[_id].withdrawnTimestamp + (((30) * 86400) * Loans[_id].repaidLoansAmount) >= block.timestamp)) {
                uint256 amount = ((Loans[_id].withdrawnAmount / paymentFrequency) + ((Loans[_id].withdrawnAmount * interestRate / 1000) / paymentFrequency));
        ...
        else if((Loans[_id].withdrawnTimestamp + ((30 * 86400) * Loans[_id].repaidLoansAmount) < block.timestamp)) {
