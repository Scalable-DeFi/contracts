...
contract MasterLending is Ownable, ReentrancyGuard{
    ...
    function withdrawFromPool(uint256 _withdrawnAmount) public onlyBorrower onlyContractEnabled nonReentrant {
        ...
        Loans[_loansIds.current()].withdrawnAmount = _withdrawnAmount;
        Loans[_loansIds.current()].withdrawnTimestamp = block.timestamp;
    ...
    function repayLoan(uint256 _id, uint256 USDCAmount) public onlyBorrower onlyContractEnabled nonReentrant {
                ...
                Loans[_id].repaidLoansValue.push(USDCAmount);
